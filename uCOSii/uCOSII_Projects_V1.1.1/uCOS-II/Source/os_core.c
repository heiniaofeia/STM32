...
void  OS_Sched (void)
{
    ...
    OS_ENTER_CRITICAL();
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       *//* 如果没中断服务运行 */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  *//* 调度器没上锁 */
            OS_SchedNew();                                                                               /* 查找最高优先级就绪任务 */
            OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];                                                  /* 得到任务控制块指针 */
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     *//* 如果得到的最高优先级就绪任务不等于当前任务 */
#if OS_TASK_PROFILE_EN > 0u
                OSTCBHighRdy->OSTCBCtxSwCtr++;         /* Inc. # of context switches to this task      *//* 统计任务切换到次任务的计数器加 1 */
#endif
